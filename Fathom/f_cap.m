function result = f_cap(Y,dis,X,W,sm,iter,verb,m,loo,b)
...
if (cda>0)
   if (sm>0)
      centroids = f_centroid(siteScores,grp,2); % spatial median
   else
      centroids = f_centroid(siteScores,grp,1); % centroid
   end
else
   centroids = NaN;
end
...
if (loo>0)
   ...
   for omit = 1:n
      ...
      result_o    = f_cap(Y(idx,:),dis,grp(idx),Y(omit,:),sm,0,0,m,0,B(omit,:)); % loo=0
