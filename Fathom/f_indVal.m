...
%  .p      = corresponding randomized probabilities
...
if iter>0
   ...
   randIV = zeros(iter-1,nc); % preallocate results array
   
   for i = 1:(iter-1) % observed value is considered a permutation
      perm        = f_indVal(Y,f_shuffle(grp),0);   % permute obs (rows)
      randIV(i,:) = perm.indVal;                    % permuted indicator values
   end
   j = ( (randIV - repmat(indVal,iter-1,1) ) >= 0); % get randomized stats >= to observed statistic
   p = (sum(j)+1)./(iter);                          % count values & convert to probability
else
   p = NaN;
end
%-----------------------------
