function mds = f_nmds(dis,dim,initial,diag,iter,tol,labels)
% - Nonmetric Multidimensional Scaling (nMDS)
%
% Usage: mds = f_nmds(dis,dim,initial,diag,iter,tol,labels);
%
% -----Input:-----
...
%
% -----Output:-----
% mds.scores = coordinates of solution
% mds.stress = final stress of solution
% mds.dim    = # of dimensions of solution
% mds.rsq    = rank Mantel statistic (fitted distance vs. original dissimilarities)
%
...
if (size(dis,1) == size(labels(:),1))<1
   error('# of rows in DIS must equal # of LABELS !');
end
...
n = size(dis,1); % size of input dissimilarity matrix
